...
def main() -> None:
    ...
    for side, can_channel, port in server_specs:
        ...
        print(
            f"[FollowerServers] starting {side} arm on {DEFAULT_SERVER_HOST}:{port}: "
            f"{' '.join(cmd)}",
            flush=True,
        )
    ...
    print("[FollowerServers] Both follower servers are ready.", flush=True)
    ...
    while True:
        for process in processes:
            ...
            if return_code is not None:
                terminate_processes(processes)
                raise RuntimeError(
                    f"Follower server process exited unexpectedly with code {return_code}."
                )
        time.sleep(1.0)
