...
class PreTrainedConfig(HFPreTrainedConfig):
    @classmethod
...
class PI05Config(PreTrainedConfig):
    ...
    def __post_init__(self):
        ...
        original_input_features = self.input_features
        original_output_features = self.output_features
        super().__init__()
        if self.dtype is None:
            self.dtype = original_dtype
        if not self.input_features:
            self.input_features = original_input_features
        if not self.output_features:
            self.output_features = original_output_features
