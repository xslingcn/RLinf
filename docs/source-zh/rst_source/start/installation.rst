...
具身智能镜像中包含多个 Python 虚拟环境（venv），位于 ``/opt/venv`` 目录下，分别对应不同模型，例如 ``openvla``、``openvla-oft``、``gr00t`` 和 ``dexbotic``。
默认环境设置为 ``openvla``。
...
- ``embodied`` target（具身智能相关），支持通过 `--model` 参数选择不同模型，例如 ``openvla``、 ``openvla-oft``、 ``gr00t`` 或 ``dexbotic``
