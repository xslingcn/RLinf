...
本示例使用 RLinf 自定义环境 `Isaac-Stack-Cube-IK-Rel-Visuomotor-Rewarded-v0` 进行强化学习训练。
如需使用该自定义环境，请按照 **依赖安装** 章节完成环境配置；该环境已默认集成在 RLinf 源码对应的 IsaacLab 库中。

- **环境**：IsaacLab 仿真平台
- **任务**：控制机械臂按蓝、红、绿顺序（自下而上）堆叠方块
...
3. **配置任务 ID**：参考 ``examples/embodiment/config/env/isaaclab_stack_cube.yaml``，修改 ``init_params.id`` 为自定义 IsaacLab 任务 ID，并确保训练配置开头的 ``defaults`` 引用了正确的环境配置。
...
本示例默认配置文件示意为 ``examples/embodiment/config/isaaclab_stack_cube_ppo_gr00t.yaml``。
